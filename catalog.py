...
def main(session_state):
    ...
    if prod_typ == 'kurtis':
        st.spinner()
        with st.spinner(text='In progress...loading ' + str(session_state.kurtis_count) + ' kurtis..'):
            time.sleep(2)
            ##st.success('Page loaded successfully')
    ...
    if prod_typ == 'sarees':
        st.spinner()
        with st.spinner(text='In progress...loading ' + str(session_state.sarees_count) + ' sarees..'):
            time.sleep(2)
            ##st.success('Page loaded successfully')
...
def load_products(prod_typ, session_state):
    ...
    for i in range(1,max_row):
        ...
        if sheet.cell(row=i+1, column=6).value in prod_typ and (sheet.cell(row=i+1, column=9).value in session_state.prod_arr[0] or sheet.cell(row=i+1, column=9).value in session_state.prod_arr[1] or sheet.cell(row=i+1, column=9).value in session_state.prod_arr[2] or sheet.cell(row=i+1, column=9).value in session_state.prod_arr[3]):
            ...
            mid.markdown('<h4>'+sheet.cell(row=i+1, column=2).value + '</h4><sub><a href="#top">Scroll to Top</a><br><span style="color: #333333;">'+ str(sheet.cell(row=i+1, column=10).value)+'</span></sub>', unsafe_allow_html=True)
