    ...
    if session_state.successful_order == False:
        if session_state.prod_arr != [[], [], [], []]:
            ...
            if session_state.user_status == 'non_verify' or session_state.mobile == '':
                ...
                if session_state.user_status == 'non_verify':
                    otp = col2.text_input("Enter OTP - sent on mobile number", key='otp')

                if session_state.user_status != 'verified' or session_state.user_status != 'verified_as_guest':
                    entry_as_guest = st.button('SKIP OTP verification')
                    info_str = '''
                    <p>if clicked on <strong>'SKIP OTP Verification'</strong>, you will be treated as <strong>Guest user</strong> and <span style="color: #800000;"><em>will be receiving call from our support team for order verification</em></span>.</p>
                    <p><span style="color: #000080;"><strong>Order will be placed ONLY when verification is successful.</strong></span></p>
                    '''
                    st.markdown(info_str, unsafe_allow_html = True)

                    if entry_as_guest:
                        session_state.user_status = 'verified_as_guest'
                ...
                if session_state.user_status == 'verified_as_guest':
                    col2.success("This mobile is unverified and treated as \'Guest\', enter other details.")
                else:
                    col2.success("Mobile number successfully verified, enter other details.")
            ...
            for i in range(1,max_row):
                if sheet.cell(row=i+1, column=9).value in clubbed_arr:
                    ...
                    item_info.append([str(sheet.cell(row=i + 1, column=1).value), str(sheet.cell(row=i + 1, column=4).value), str(sheet.cell(row=i + 1, column=6).value), qty, size, final_actual_cost])
                    st.markdown("""<hr>""", unsafe_allow_html=True)
            ...
            if create_order:
                ...
                if ok_for_order == True:
                    ...
                    cart_id = str(datetime.datetime.now()).replace('-', '').replace(':', '').replace(' ', '').replace('.', '')
                    cur_date = datetime.datetime.now().date()

                    #Create cart entry
                    cart_data = helper.create_cart_entry(session_state, cur_date, cart_id, name, pincode, email, address, total_final_actual_cost, item_count)

                    #Create order entry
                    order_data = helper.create_order_entry(session_state, cur_date, cart_id, name, pincode, email, address, total_final_actual_cost, item_count, item_info)

                    helper.send_mail_to_backoffice(session_state, cart_data, order_data)
                    session_state.session_order_id = cart_data[0]
                    st.experimental_rerun()
                    session_state.successful_order = False

                    # Send mail for order creation with data,
                    #helper.send_mail_to_backoffice(session_state ,name, pincode, email, address)
        ...
        message_txt = '''
        <p><span style="font-size:16px;"><span style="color:#008000;"><strong>Order created successfully! ORDER ID is '''+ session_state.session_order_id +'''</strong></span></span></p>
        <p><span style="color:#0000FF;">Copy and save this ID for future reference. </span></p>
        <p><span style="color:#0000FF;">In &#39;<em><strong>Check Order Status</strong></em>&#39; in sidebar, enter this ID and check status.</span></p>
        '''
        st.markdown(message_txt, unsafe_allow_html = True)
        st.warning('There is no product in cart.')
